...
const LatestJobCard = ({job}) => {
  ...
  return (
    <div onClick={()=>navigate(`/jobDetail/${job?._id}`)} className="hover:scale-105 transition-transform duration-200 cursor-pointer">
      <Card className="mt-3 w-90 shadow-md border border-gray-100 rounded-md">
        <CardBody>
          <Typography variant="h5" color="blue-gray" className="mb-2">
            {job?.company?.name}
          </Typography>
          <h3 className=" mb-2  text-gray-400 text-muted-foreground">{job?.location}</h3>
          <Typography variant="h5" color="blue-gray" className="mb-2">
            {job?.title}
          </Typography>
          <Typography>
